...
// ── PASS-C: MAP ARCHETYPES ──────────────────────────────────────────────
// Each match rolls one of these. Lane/bridge/tower geometry is per-map.
// Tower X positions (player/enemy) are auto-mirrored at startMatch time.
// `lanes` is an array of Z coords. `river.half` = 0 means no river.
const MAP_ARCHETYPES = {
  three_lanes: {
    name: '3-LANE CLASSIC',
    desc: 'River split, 3 bridges, 3 lanes. Default battle.',
    lanes: [-9.5, 0, 9.5],
    laneHalf: 3.6,
    river: { half: 1.0 },
    bridges: [{ z:-9.5, halfLength:2.2 }, { z:0, halfLength:2.2 }, { z:9.5, halfLength:2.2 }],
    towerZs: [-9.5, 0, 9.5],
    towerX: 22,
    treeWalls: [
      { zMin:-18, zMax:-11.5 },
    ...
    capturePoint: { x:0, z:0 },
  },
  open_field: {
    name: 'OPEN FIELD',
    desc: 'No lanes. Free movement. Scattered cover.',
    lanes: [],
    laneHalf: 0,
    river: { half: 0.9 },
    bridges: [{ z:-12, halfLength:3 }, { z:-4, halfLength:3 }, { z:4, halfLength:3 }, { z:12, halfLength:3 }],
    towerZs: [-11, 0, 11],
    towerX: 22,
    treeWalls: [],
    capturePoint: { x:0, z:0 },
    extras: { obstacles:'rocks_dense' },
  },
  bridgehead: {
    name: 'BRIDGEHEAD',
    desc: 'Single chokepoint. One wide bridge. Defender advantage.',
    lanes: [0],
    laneHalf: 5,
    river: { half: 1.6 },
    bridges: [{ z:0, halfLength:5.5 }],
    towerZs: [-7, 0, 7],   // central + 2 flanks
    towerX: 22,
    treeWalls: [
      { zMin:-18, zMax:-6 },
      { zMin:6, zMax:18 },
  ...
  crossroads: {
    name: 'CROSSROADS',
    desc: '4 lanes meet at center. No river — full open exchanges.',
    lanes: [-13, -4, 4, 13],
    laneHalf: 3.0,
    river: { half: 0 },
    bridges: [],
  ...
  twin_forks: {
    name: 'TWIN FORKS',
    desc: '2 wide lanes. River split. Pick your push.',
    lanes: [-7, 7],
    laneHalf: 4,
    river: { half: 1.0 },
    bridges: [{ z:-7, halfLength:2.8 }, { z:7, halfLength:2.8 }],
    towerZs: [-7, 7],
    towerX: 22,
    treeWalls: [
      { zMin:-18, zMax:-11 },
  ...
  highground: {
    name: 'HIGHGROUND',
    desc: 'Elevated mesa at center. Hold the hill.',
    lanes: [-9, 0, 9],
    laneHalf: 3.4,
    river: { half: 1.0 },
    bridges: [{ z:-9, halfLength:2.2 }, { z:0, halfLength:2.2 }, { z:9, halfLength:2.2 }],
    towerZs: [-9, 0, 9],
    towerX: 22,
    treeWalls: [
      { zMin:-18, zMax:-11 },
    ...
    capturePoint: { x:0, z:0, bonus:2 },   // Holding the hill is more valuable
    extras: { highground:{ x:0, z:0, radius:8, height:1.4 } },
  },
};

// ── PASS-C: WEATHER ────────────────────────────────────────────────────
// Weather rolls per match. Each affects gameplay subtly.
const WEATHERS = {
  clear: { name:'CLEAR', desc:'Perfect visibility', rangeMul:1.0, sightMul:1.0, fogDensity:0, lightning:false, ashCloud:false },
  fog:   { name:'FOG',   desc:'Range halved',      rangeMul:0.55, sightMul:0.7, fogDensity:1.7, lightning:false, ashCloud:false },
  storm: { name:'STORM', desc:'Lightning hits random units',  rangeMul:0.85, sightMul:0.9, fogDensity:1.2, lightning:true, ashCloud:false },
  ash:   { name:'ASH CLOUD', desc:'No air units allowed',     rangeMul:0.95, sightMul:0.85, fogDensity:1.4, lightning:false, ashCloud:true },
};
function getWeather(){ return WEATHERS[STATE.weatherKey || 'clear'] || WEATHERS.clear; }

// Daily seed RNG — deterministic for "Battle of the Day"
let _rngState = 0;
function rngSeed(seed){ _rngState = seed >>> 0; if (_rngState === 0) _rngState = 1; }
function rng(){
  // xorshift32
  _rngState ^= _rngState << 13; _rngState >>>= 0;
  _rngState ^= _rngState >>> 17; _rngState >>>= 0;
...
// Helpers that read from STATE.map (with fallbacks for legacy paths)
function currentLanes(){ return (STATE.map && STATE.map.lanes && STATE.map.lanes.length) ? STATE.map.lanes : [LANE_TOP_Z, LANE_MID_Z, LANE_BOT_Z]; }
function currentLaneHalf(){ return STATE.map ? STATE.map.laneHalf : LANE_HALF; }
function currentBridges(){ return (STATE.map && STATE.map.bridges) ? STATE.map.bridges : BRIDGES; }
function currentRiverHalf(){ return (STATE.map && STATE.map.river) ? STATE.map.river.half : RIVER_HALF; }

// Pass-D: more saturated themes so units pop against terrain
const MAP_THEMES = {
  grass:  { name:'FORESTED', ground:0x5aa040, dirtPath:0x9a6830, river:0x2870c0, bridge:0x8a6840, trunk:0x4a2a15, foliage:[0x3a8030,0x4a8838,0x5a9840], rock:0x807268, bush:0x4a8030, distant:0x1a4218, flowers:[0xfff060,0xff7090,0xffffff,0xc880ff] },
  desert: { name:'DESERT',   ground:0xe0a040, dirtPath:0xa86028, river:0x3a8090, bridge:0x6a4a20, trunk:0x5a3a20, foliage:[0x70a040,0x80b048,0x90c050], rock:0xb89058, bush:0x90702a, distant:0x4a3a18, flowers:[0xffe040,0xff9858,0xffe070,0xf07840] },
  snow:   { name:'SNOW',     ground:0xeef4fa, dirtPath:0x88a0b8, river:0x4080b0, bridge:0x504848, trunk:0x4a3828, foliage:[0x4a8040,0x3a7038,0x5a9050], rock:0xb0b8c0, bush:0xa8b8c8, distant:0x4a6858, flowers:[0xffffff,0xe8a8ff,0xffe0e8,0xc0c0f0] },
  urban:  { name:'URBAN',    ground:0x6a7068, dirtPath:0x383830, river:0x1a3850, bridge:0x383838, trunk:0x2a2a2a, foliage:[0x4a8030,0x5a9038,0x6aa040], rock:0x585858, bush:0x4a6020, distant:0x303038, flowers:[0xff5858,0xffd040,0x40a8ff,0xff58d0] },
};
...
const STATE = {
  ...
  // Pass-C state
  map:null, weatherKey:'clear', useDailySeed:false,
  capture:{ side:null, progress:0, holder:null },   // capture point
  supplyDrops:[],   // active parachute crates
  supplyTimer:30,   // seconds until next drop
  lightningTimer:8, // seconds until next storm strike
...
const PROJ_POOL_CAP = 32;   // per-kind soft cap; beyond this we just GC
...
// Pass-D: Outlines were the biggest perf killer — every mesh part got a
// back-faced duplicate (~25 parts × 2 = 50 draw calls per infantry unit).
// New: only outline meshes WITH bounding sphere radius >= MIN_R. Skips
// tiny parts (eyes, handles, decorations) which are barely visible
// anyway. Cuts draw calls by 60-70% for character meshes.
const _OUTLINE_MIN_R = 0.18;   // bounding-sphere radius threshold
const _OUTLINE_MAT = new THREE.MeshBasicMaterial({ color:0x080808, side:THREE.BackSide });
function addOutline(mesh, thickness){
  thickness = thickness || 1.05;
  mesh.traverse(o => {
    if (!o.isMesh || o.userData.isOutline || !o.geometry) return;
    // Quick size check via bounding sphere
    if (!o.geometry.boundingSphere) o.geometry.computeBoundingSphere();
    const r = (o.geometry.boundingSphere && o.geometry.boundingSphere.radius) || 0;
    if (r < _OUTLINE_MIN_R) return;   // skip tiny parts
    const out = new THREE.Mesh(o.geometry, _OUTLINE_MAT);
    out.scale.setScalar(thickness);
    out.userData.isOutline = true;
    out.renderOrder = -1;
    out.castShadow = false;          // outlines never cast shadows
...
function buildField(theme){
  ...
  // Lane paths overlay
  // Pass-C: dynamic lane paths from current map
  // Pass-D: distinct per-lane colored bands so each lane is visually unique.
  // Stronger contrast, slight color shift per lane (top=cool, mid=neutral, bot=warm).
  const lanes = currentLanes();
  const laneTints = [0xff8060, 0xfff088, 0x80c0ff, 0x80ff88, 0xff80c8];   // cycling
  for (let li = 0; li < lanes.length; li++) {
    const lz = lanes[li];
    // Dirt path base
    const path = new THREE.Mesh(
      new THREE.PlaneGeometry(FIELD_W - 14, 3.4),
      new THREE.MeshBasicMaterial({ color:theme.dirtPath, transparent:true, opacity:0.55, depthWrite:false })
    );
    path.rotation.x = -Math.PI/2; path.position.set(0, 0.04, lz);
    scene.add(path);
    // Per-lane color edge (subtle but readable from the top)
    const tint = laneTints[li % laneTints.length];
    for (const zo of [-1.7, 1.7]) {
      const edge = new THREE.Mesh(
        new THREE.PlaneGeometry(FIELD_W - 14, 0.18),
        new THREE.MeshBasicMaterial({ color:tint, transparent:true, opacity:0.55, depthWrite:false })
      );
      edge.rotation.x = -Math.PI/2; edge.position.set(0, 0.05, lz + zo);
      scene.add(edge);
    }
  }
  // Side tints — adapt for no-river maps
  const rH = currentRiverHalf();
  for (const [side, col] of [[-1, 0x3070b0], [1, 0xb04030]]) {
    const w = FIELD_W/2 - rH;
    const tint = new THREE.Mesh(
      new THREE.PlaneGeometry(Math.max(1, w), FIELD_D),
      new THREE.MeshBasicMaterial({ color:col, transparent:true, opacity:0.06, depthWrite:false })
    );
    tint.rotation.x = -Math.PI/2;
    tint.position.set(side * (FIELD_W/4 + rH/2), 0.03, 0);
    scene.add(tint);
  }
  // Deploy zone marker
  deployZoneMesh = new THREE.Mesh(
    new THREE.PlaneGeometry(Math.max(1, FIELD_W/2 - rH - 4), FIELD_D - 2),
  ...
  // Bridges (Pass-C: dynamic per map, none if no river)
  for (const b of currentBridges()) {
    const bg = new THREE.Group();
    const bridgeWidth = Math.max(1.0, rH * 2.8);
    const deck = new THREE.Mesh(new THREE.BoxGeometry(bridgeWidth, 0.18, b.halfLength * 2), toonMat(theme.bridge));
    deck.position.y = 0.1; deck.castShadow = true; deck.receiveShadow = true; bg.add(deck);
    for (let zs of [-b.halfLength, b.halfLength]) {
      const rail = new THREE.Mesh(new THREE.BoxGeometry(bridgeWidth, 0.32, 0.08), toonMat(0x4a3020));
      rail.position.set(0, 0.34, zs); bg.add(rail);
    }
    // Posts
    for (let zs of [-b.halfLength + 0.4, b.halfLength - 0.4]) {
      for (let xs of [-rH * 1.3, rH * 1.3]) {
...
function buildWater(theme, tod){
  ...
  // Pass-C: river-less maps skip the water entirely
  const rH = currentRiverHalf();
  if (rH <= 0) { waterMesh = null; return; }
  const waterGeo = new THREE.PlaneGeometry(rH * 2, FIELD_D + 30, 2, 40);
...
function rebuildTrees(theme){
  ...
  const Q = getQuality();
  // Pass-C: tree walls come from the current map archetype.
  // If the map has none (open field), scatter sparse cover instead.
  const map = STATE.map;
  const rH = currentRiverHalf();
  let walls;
  if (map && map.treeWalls && map.treeWalls.length) {
    walls = map.treeWalls;
  } else {
    // Open field — sprinkle scattered clumps as cover (one big "wall" with random skips)
    walls = [{ zMin:-FIELD_D/2 + 1, zMax:FIELD_D/2 - 1, sparse:true }];
  }
  for (const wall of walls) {
    const sparse = wall.sparse ? 0.18 : 1.0;   // sparse maps drop most trees
    for (let x = -FIELD_W/2 + 5; x <= FIELD_W/2 - 5; x += 1.7) {
      if (Math.abs(x) < rH + 1.8) continue;
      for (let zRow = wall.zMin; zRow <= wall.zMax; zRow += 1.8) {
        const jx = x + (Math.random() - 0.5) * 1.0;
        const jz = zRow + (Math.random() - 0.5) * 0.9;
        if (Math.abs(jx) < rH + 1.3) continue;
        if (Math.random() > Q.drawTrees * sparse) continue;
...
function buildUnitMesh(def, side){
  ...
  // Pass-D: BIG team-color disc under every unit. Top-down camera makes
  // these the primary identifier of who's on which side.
  const teamCol = teamColor(side);
  const radius = def.type === 'air' ? 1.0 : (def.roleKey === 'tank' || def.roleKey === 'apc' ? 1.4 : def.roleKey === 'commander' ? 1.1 : 0.85);
  const disc = new THREE.Mesh(
    new THREE.RingGeometry(radius * 0.78, radius, 24),
    new THREE.MeshBasicMaterial({ color:teamCol, transparent:true, opacity:0.85, side:THREE.DoubleSide, depthWrite:false })
  );
  ...
  disc.renderOrder = -2;
  mesh.add(disc);
  // Inner glow disc — slightly larger, faded
  const glow = new THREE.Mesh(
    new THREE.CircleGeometry(radius * 0.72, 18),
    new THREE.MeshBasicMaterial({ color:teamCol, transparent:true, opacity:0.20, depthWrite:false })
  );
...
function getLaneForZ(z){
  // Pass-C: dynamic lanes from STATE.map. If no lanes (open field), return
  // a virtual "free" lane that just clamps to the position.
  const lanes = currentLanes();
  if (!lanes.length) return { id:'free', z };
  let best = { id:0, z:lanes[0] }, bd = Math.abs(z - lanes[0]);
  for (let i = 1; i < lanes.length; i++) {
    const d = Math.abs(z - lanes[i]);
    if (d < bd) { bd = d; best = { id:i, z:lanes[i] }; }
  }
  return best;
}
function spawnUnit(unitKey, side, fx, fz){
  ...
  // Pass-C: only snap to lane if the map actually has lanes (open field skips this)
  if (def.type === 'ground' && def.speed > 0 && currentLanes().length) {
    const lane = getLaneForZ(fz);
    const lh = currentLaneHalf();
    fz = lane.z + (fz - lane.z) * 0.3;
    fz = Math.max(lane.z - lh * 0.85, Math.min(lane.z + lh * 0.85, fz));
  }
  for (let i = 0; i < count; i++) {
    ...
    const u = {
      ...
      // Pass-D lane leash: remember the Z where this unit was deployed.
      // findTarget will only acquire enemies near this Z band, and the
      // unit never strays more than ±LANE_LEASH from this Z.
      homeLaneZ: fz,
      // Cached effective stats — read by movement/fire instead of def.* directly
      effSpeed: def.speed * fm.speedMul,
      effRange: def.range * fm.rangeMul * (getWeather().rangeMul || 1),
      effDmgMul: fm.dmgMul,
...
function createTowers(){
  ...
  // Pass-C: tower X comes from STATE.map (per-archetype). HQ X is fixed.
  const map = STATE.map || { towerZs:[LANE_TOP_Z, LANE_MID_Z, LANE_BOT_Z], towerX:22 };
  const towerZs = map.towerZs;
  const pTX = -map.towerX, eTX = map.towerX;
  ...
  // Tower type per slot — first/last get the player's chosen top/bottom; rest get gun
  function pickType(i, n) {
    if (n === 1) return midT;
    if (i === 0) return topT;
    if (i === n - 1) return botT;
    return midT;
  }
  function laneLabel(i, n) {
    if (n === 1) return 'M';
    if (i === 0) return 'T';
    if (i === n - 1) return 'B';
    return 'M' + i;
  ...
  for (let i = 0; i < towerZs.length; i++) {
    const z = towerZs[i];
    const type = pickType(i, towerZs.length);
    const lab = laneLabel(i, towerZs.length);
    list.push(mk('player', pf, type, pTX, z, lab, 1));
    list.push(mk('enemy', ef, type, eTX, z, lab, ehp));
  }
  for (const t of list) {
    t.id = nextId++;
    STATE.towers.push(t);
...
// ── PASS-C: CAPTURE POINT ──────────────────────────────────────────────
// Midfield circle. While units of one side are nearby (and other side has
// none), that side gains capture progress. Holder gets +1/+2 energy/sec.
let _captureMesh = null, _captureFlag = null;
// Pass-C: re-do the static field meshes when the rolled map differs from
// what's currently in the scene. Cheap — just removes & re-adds.
let _fieldStaticMeshes = [];
function reinitFieldForMap(){
  if (!scene) return;
  // Tag and dispose previously-built field statics. Safer to just remove
  // the heavy meshes (ground, water, paths, river tints, bridges) we re-create.
  // We track them in _fieldStaticMeshes added during buildField.
  for (const m of _fieldStaticMeshes) removeAndDispose(m);
  _fieldStaticMeshes = [];
  if (waterMesh) { removeAndDispose(waterMesh); waterMesh = null; }
  if (deployZoneMesh) { removeAndDispose(deployZoneMesh); deployZoneMesh = null; }
  if (rangePreviewMesh) { removeAndDispose(rangePreviewMesh); rangePreviewMesh = null; }
  if (groundMesh) { removeAndDispose(groundMesh); groundMesh = null; }
  buildField(getTheme());
  buildWater(getTheme(), getTOD());
}

function spawnCapturePoint(){
  if (_captureMesh) { removeAndDispose(_captureMesh); _captureMesh = null; }
  if (!STATE.map || !STATE.map.capturePoint) return;
  const cp = STATE.map.capturePoint;
  STATE.capture = { side:null, progress:0, holder:null, x:cp.x, z:cp.z, radius:3.5, bonus: cp.bonus || 1 };
  // Visual: glowing ring + flag pole
  const g = new THREE.Group();
  const ring = new THREE.Mesh(
    new THREE.RingGeometry(3.4, 3.7, 36),
    new THREE.MeshBasicMaterial({ color:0xfff088, transparent:true, opacity:0.65, side:THREE.DoubleSide, depthWrite:false })
  );
  ring.rotation.x = -Math.PI/2; ring.position.y = 0.06; g.add(ring);
  const inner = new THREE.Mesh(
    new THREE.CircleGeometry(3.4, 24),
    new THREE.MeshBasicMaterial({ color:0xffe040, transparent:true, opacity:0.10, depthWrite:false })
  );
  inner.rotation.x = -Math.PI/2; inner.position.y = 0.05; g.add(inner);
  // Flag pole — color shifts with holder
  const pole = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 2.4, 6), toonMat(0x202020));
  pole.position.y = 1.2; g.add(pole);
  const flag = new THREE.Mesh(new THREE.BoxGeometry(0.7, 0.45, 0.04), basicMat(0x808080));
  flag.position.set(0.35, 2.0, 0); g.add(flag);
  _captureFlag = flag;
...
function tickCapture(dt){
  ...
  const cp = STATE.capture;
  if (!cp.x && cp.x !== 0) return;
  // Count units of each side near the point
  let p = 0, e = 0;
  for (const u of STATE.units) {
    if (u.hp <= 0) continue;
    if (Math.hypot(u.x - cp.x, u.z - cp.z) > cp.radius) continue;
    if (u.side === 'player') p++; else e++;
  }
  let target = null;
  if (p > 0 && e === 0) target = 'player';
  else if (e > 0 && p === 0) target = 'enemy';
  // Capture progress (0..1). Holder is set when progress reaches 1.
  if (target === cp.holder) {
    cp.progress = Math.min(1, cp.progress + dt * 0.4);
  } else if (target) {
    cp.progress = Math.max(0, cp.progress - dt * 0.6);
    if (cp.progress === 0) cp.holder = target;
  } else {
    cp.progress = Math.max(0, cp.progress - dt * 0.15);
  }
  // Holder gets bonus energy/sec
  if (cp.holder && cp.progress >= 1) {
    const e2 = cp.bonus * dt * (STATE.energyMul || 1);
    if (cp.holder === 'player') STATE.energy = Math.min(STATE.maxEnergy, STATE.energy + e2);
    else STATE.enemyEnergy = Math.min(STATE.maxEnergy, STATE.enemyEnergy + e2);
  }
  // Update flag color
  if (_captureFlag) {
    let col = 0x808080;
    if (cp.holder === 'player') col = 0x4080d0;
    else if (cp.holder === 'enemy') col = 0xd04030;
    _captureFlag.material.color.setHex(col);
    // Flag pulses when contested
    if (target && target !== cp.holder) _captureFlag.scale.x = 1 + Math.sin(performance.now() * 0.008) * 0.2;
    else _captureFlag.scale.x = 1;
  }
}

// ── PASS-C: SUPPLY DROPS ───────────────────────────────────────────────
// Crate parachutes in midfield. First side to reach it claims the bonus
// (+3 energy, or in 30% of cases a free random unit at that location).
function spawnSupplyDrop(){
  // Pick a random midfield-ish spot
  const z = (Math.random() - 0.5) * (FIELD_D - 8);
  const x = (Math.random() - 0.5) * 16;   // around center
  const drop = { id:nextId++, x, z, y:24, vy:-7, claimed:false, bonus: Math.random() < 0.3 ? 'unit' : 'energy', bonusAmt: 3 };
  // Visual: crate + parachute
  const g = new THREE.Group();
  const crate = new THREE.Mesh(new THREE.BoxGeometry(0.7, 0.7, 0.7), toonMat(0xa06030));
  crate.position.y = 0.35; crate.castShadow = true; g.add(crate);
  // Slats
  for (let yp of [0.10, 0.60]) {
    const s = new THREE.Mesh(new THREE.BoxGeometry(0.72, 0.05, 0.72), toonMat(0x402010));
    s.position.y = yp; g.add(s);
  }
  // Stripe
  const sup = new THREE.Mesh(new THREE.BoxGeometry(0.72, 0.18, 0.04), basicMat(0xfff088));
  sup.position.set(0, 0.35, 0.36); g.add(sup);
  // Parachute (sphere top, removed on land)
  const chute = new THREE.Mesh(new THREE.SphereGeometry(1.4, 14, 8, 0, Math.PI*2, 0, Math.PI/2.2), basicMat(0xff8040, { transparent:true, opacity:0.85 }));
  chute.position.y = 1.6; g.add(chute);
  drop.chute = chute; drop.crate = crate;
  // Glow ring on ground
  const ring = new THREE.Mesh(new THREE.RingGeometry(1.2, 1.4, 22), basicMat(0xfff088, { transparent:true, opacity:0.7, side:THREE.DoubleSide, depthWrite:false }));
  ring.rotation.x = -Math.PI/2; ring.position.y = 0.02; g.add(ring);
  drop.ring = ring;
  g.position.set(x, drop.y, z);
  drop.mesh = g;
  scene.add(g);
...
function tickSupplyDrops(dt){
  if (!STATE.currentMission) return;
  STATE.supplyTimer -= dt;
  if (STATE.supplyTimer <= 0) {
    spawnSupplyDrop();
    STATE.supplyTimer = 30 + Math.random() * 25;
  }
  for (const d of STATE.supplyDrops) {
    ...
    if (d.y > 0.4) {
      ...
      // Parachute swing
      if (d.chute) d.chute.rotation.z = Math.sin(performance.now() * 0.003) * 0.15;
      // Pop chute when landed
      if (d.y === 0.4 && d.chute) { d.mesh.remove(d.chute); d.chute = null; addFX({ type:'puff', x:d.x, y:0.3, z:d.z, t:0, dur:0.6 }); }
    } else {
      // Pulsing glow on ground
      if (d.ring) {
        d.ring.scale.setScalar(1 + Math.sin(performance.now() * 0.005) * 0.18);
        d.ring.material.opacity = 0.5 + Math.sin(performance.now() * 0.005) * 0.2;
      }
      // Check claim
      let nearestSide = null, nearestD = Infinity;
      for (const u of STATE.units) {
        if (u.hp <= 0) continue;
        const dd = Math.hypot(u.x - d.x, u.z - d.z);
        if (dd < 1.2 && dd < nearestD) { nearestD = dd; nearestSide = u.side; }
      }
      if (nearestSide) {
        d.claimed = true;
        if (d.bonus === 'energy') {
          if (nearestSide === 'player') { STATE.energy = Math.min(STATE.maxEnergy, STATE.energy + d.bonusAmt); showToast('SUPPLY CLAIMED +3 ENERGY'); }
          else STATE.enemyEnergy = Math.min(STATE.maxEnergy, STATE.enemyEnergy + d.bonusAmt);
        } else {
          // Spawn a free random unit at the drop site
          const fk = nearestSide === 'player' ? STATE.progress.playerFaction : (STATE.currentMission ? STATE.currentMission.enemyFaction : 'empire');
          const pool = ['rifleman','heavygunner','lightV','flamer','sniper'];
          const pick = pool[Math.floor(Math.random() * pool.length)];
          spawnUnit(fk + '_' + pick, nearestSide, d.x, d.z);
          if (nearestSide === 'player') showToast('SUPPLY CLAIMED — FREE UNIT');
        }
        // Pop animation
        addFX({ type:'flash', x:d.x, y:0.5, z:d.z, t:0, dur:0.3, size:1.2 });
        addFX({ type:'ring', x:d.x, y:0.1, z:d.z, t:0, dur:0.7, size:2.5, color:0xfff088 });
        playSound('paycheck');
        if (d.mesh) removeAndDispose(d.mesh);
        d.mesh = null;
      }
    }
  }
  STATE.supplyDrops = STATE.supplyDrops.filter(d => !d.claimed);
}

// ── PASS-C: WEATHER (lightning + ash + fog effect on Three.js fog) ─────
function applyWeatherToScene(){
  const w = getWeather();
  if (!scene || !scene.fog) return;
  const tod = getTOD();
  const baseFar = tod.fogFar;
  const baseNear = tod.fogNear;
  scene.fog.near = baseNear / Math.max(0.5, w.fogDensity || 1);
  scene.fog.far  = baseFar / Math.max(0.5, w.fogDensity || 1);
}
function tickWeather(dt){
  ...
  if (w.lightning) {
    ...
    if (STATE.lightningTimer <= 0) {
      STATE.lightningTimer = 6 + Math.random() * 8;
      // Pick a random unit to hit
      const cands = STATE.units.filter(u => u.hp > 0);
      if (cands.length) {
        const tgt = cands[Math.floor(Math.random() * cands.length)];
        // Telegraph + bolt
        addFX({ type:'beam', x:tgt.x, y:30, z:tgt.z, tx:tgt.x, tz:tgt.z, t:0, dur:0.4, color:0xa0c8ff });
        addFX({ type:'flash', x:tgt.x, y:1, z:tgt.z, t:0, dur:0.4, size:2 });
        applyDamage(tgt, 220);
  ...
  // ASH CLOUD: kill any active air units (and prevent new ones)
  if (w.ashCloud) {
    for (const u of STATE.units) {
      if (u.hp > 0 && u.def && u.def.type === 'air') applyDamage(u, 999);
    }
  }
}
function applyWeatherToRange(u){
  // Multiplied range (used by acquire); we mutate effRange at spawn time.
  const w = getWeather();
  return (w.rangeMul || 1);
}
...
function clearBattle(){
  ...
  // Pass-C: clean up capture point + supply drops + storm/lightning
  if (_captureMesh) { removeAndDispose(_captureMesh); _captureMesh = null; }
  _captureFlag = null;
  STATE.capture = { side:null, progress:0, holder:null };
  for (const d of (STATE.supplyDrops||[])) if (d.mesh) removeAndDispose(d.mesh);
  STATE.supplyDrops = [];
  STATE.supplyTimer = 25 + Math.random() * 15;
  STATE.lightningTimer = 6 + Math.random() * 6;
  STATE.stats = { kills:0, allyDeaths:0, deployed:0, dmgDealt:0, dmgTaken:0 };
}

function needsBridge(u, t){
  // Pass-C: only need a bridge if there's a river to cross
  if (currentRiverHalf() <= 0) return false;
  return u.def.type === 'ground' && ((u.x < 0) !== (t.x < 0));
}
function nearestBridge(z){
  const bridges = currentBridges();
...
function moveToward(u, target, dt){
  ...
  const riverHalf = currentRiverHalf();
  const bridges = currentBridges();
  if (needsBridge(u, { x:goalX, z:goalZ }) && bridges.length) {
    const onBridge = Math.abs(u.x) < riverHalf + 0.6 && bridges.some(b => Math.abs(u.z - b.z) < b.halfLength);
    if (!onBridge) {
      const br = bridges.find(b => Math.abs(b.z - myLane.z) < 2.0) || nearestBridge(u.z);
      if (br) {
        goalZ = br.z;
        goalX = (u.x < 0) ? -riverHalf - 0.4 : riverHalf + 0.4;
      }
    }
  }
  ...
  // River push (only if there's a river)
  if (def.type === 'ground' && riverHalf > 0 && Math.abs(u.x) < riverHalf && bridges.length) {
    const onBridge = bridges.some(b => Math.abs(u.z - b.z) < b.halfLength);
    if (!onBridge) {
      const br = bridges.find(b => Math.abs(b.z - myLane.z) < 2.0) || nearestBridge(u.z);
      if (br) u.z += Math.sign(br.z - u.z) * speed * dt * 1.2;
    }
  }
  // Tree chop
...
// Pass-D: lane-leashed target finder.
// Units only acquire enemies near their home lane Z (band of LANE_LEASH).
// This stops snipers running into forests, light vehicles abandoning lanes,
// etc. Air units ignore the leash. Stationary units only care about
// their own range — leash doesn't apply since they don't move anyway.
const LANE_LEASH = 6.5;        // ±Z from home lane the unit considers its lane
const LANE_LEASH_AIR = 999;    // air ignores leash
function findTarget(u){
  ...
  const leash = u.def.type === 'air' ? LANE_LEASH_AIR : LANE_LEASH;
  for (const e of STATE.units) {
    ...
    // Lane leash: ignore enemies whose Z is outside our lane band
  ...
  if (canG) {
    ...
    for (const t of STATE.towers) {
      ...
      // Towers ARE valid targets across lanes once HQ is exposed
      const d = dist(u, t);
      if (d < bd) { bd = d; best = t; }
    }
  }
...
// ── FX (visuals) ────────────────────────────────────────────────────────
// Pass-D: FX queue cap. Without this a 4-tank firefight produces hundreds
// of debris/sparks/chunks per second and the framerate tanks.
const FX_CAP = 220;
function addFX(fx){
  const Q = getQuality();
  // Particle-style FX get statistically dropped at low quality
  if ((fx.type === 'spark' || fx.type === 'debris' || fx.type === 'splatter' || fx.type === 'chunk') && Math.random() > Q.particleMul) return;
  // Hard cap: when over budget, drop oldest non-decal FX
  if (fxObjects.length >= FX_CAP) {
    // Find first non-persistent (skip pool/bones/stain — those are decals)
    for (let i = 0; i < fxObjects.length; i++) {
      const f = fxObjects[i];
      if (f.type !== 'pool' && f.type !== 'stain' && f.type !== 'bones') {
...
// Pass-D: per-frame frustum object reused so we don't allocate per check
...
function _updateFrustum(){
  ...
  _frust.setFromProjectionMatrix(_projScreenMat);
}
// Reused vector for cheap unit visibility test
const _vTmp = new THREE.Vector3();
function unitVisible(u){
  // Cheap: test a sphere around unit position. Skips full mesh traverse.
  _vTmp.set(u.x, u.def.type === 'air' ? 3.5 : 0.5, u.z);
  return _frust.containsPoint(_vTmp) ||
         _frust.intersectsSphere(new THREE.Sphere(_vTmp, 1.2));
}
...
function syncMeshes(dt){
  ...
  for (const u of STATE.units) {
    ...
    // Pass-D frustum cull: skip the heavy animation pass if not on screen.
    // Position is still updated above (cheap), only the bone math is skipped.
    const visible = unitVisible(u);
    mesh.visible = visible;
    if (visible) animateUnit(u, mesh, dt);
    // HP pip — only when visible
...
function update(dt) {
  ...
  for (const u of STATE.units) {
    ...
    if (u.cooldown > 0) u.cooldown -= dt;
    // Pass-D: STATIONARY behavior — these units never chase. They hold
    // position, rotate to face target, and fire only when target is in
    // range. Sniper/AA/artillery now plant themselves where deployed.
    const isStationary = u.def.speed === 0 ||
      u.def.roleKey === 'sniper' || u.def.roleKey === 'aa' || u.def.roleKey === 'artillery';
    if (u.target) {
      ...
      if (d <= eRange) {
        ...
        // Out of range — stationary unit drops the target instead of chasing.
        // findTarget will reacquire something else (or nothing) next frame.
        u.target = null;
        u.moving = false;
      } else {
      ...
      // No target → push toward enemy HQ along our lane
      const eHQ = STATE.towers.find(t => t.role === 'hq' && t.side !== u.side);
      if (eHQ) {
        // Pass-D: leash to home-lane Z when pushing toward HQ — keeps
        // ground units in their lane instead of cutting across the field.
        const homeZ = u.homeLaneZ != null ? u.homeLaneZ : u.z;
        const goal = { x:eHQ.x, z: u.def.type === 'air' ? eHQ.z : homeZ };
...
function attemptDeploy(key, x, z) {
  ...
  // Pass-C: ASH CLOUD weather grounds air units
  if (getWeather().ashCloud && def.type === 'air') { showToast('AIR GROUNDED — ASH CLOUD'); playSound('ui_click'); return; }
...
// Pass-D: chunked + deferred portrait bake.
// Was: 60 portraits baked synchronously at boot → 2-3 second freeze.
// Now: 4 per microtask chunk so the page stays responsive. If bake throws
// (some browsers block toDataURL), the SVG fallback in cardIconSVG kicks in.
let _bakeQueue = null;
function bakeAllPortraits(){
  if (_bakeQueue) return;   // already running
  _bakeQueue = Object.keys(UNITS).slice();
  function chunk(){
    if (!_bakeQueue || !_bakeQueue.length) { _bakeQueue = null; return; }
    const batch = _bakeQueue.splice(0, 4);
    for (const k of batch) {
      try { const u = bakeCardPortrait(k); if (u) CARD_PORTRAITS[k] = u; }
      catch(e){ /* SVG fallback handles it */ }
    }
    // Refresh hand if we're in a match so cards update
    if (STATE.running) renderHand();
    setTimeout(chunk, 30);
  }
...
function cardIconSVG(key, size) {
  ...
  // Pass-D: render as a plain DIV with background-image (more reliable
  // across browsers than embedded SVG <image href="data:...">).
  if (UNITS[key] && CARD_PORTRAITS[key]) {
    const ws = size ? size + 'px' : '100%';
    const hs = size ? size + 'px' : '100%';
    const url = CARD_PORTRAITS[key];
    return `<div style="width:${ws};height:${hs};border-radius:50%;` +
           `background:radial-gradient(circle at 50% 40%, ${main}99 0%, ${dark}f0 100%) center/cover no-repeat;` +
           `border:1px solid ${glow};box-shadow:0 0 6px ${glow}80,inset 0 0 12px ${dark};` +
           `position:relative;overflow:hidden">` +
             `<img src="${url}" alt="" style="position:absolute;inset:5%;width:90%;height:90%;` +
             `object-fit:contain;pointer-events:none;filter:drop-shadow(0 1px 1px rgba(0,0,0,0.6))" ` +
             `onerror="this.remove()"/>` +
           `</div>`;
  }
  const role = def.roleKey;
...
function startMatch(mission) {
  ...
  // ── Pass-C: ROLL THE BOARD ─────
  // Mission can lock map ('mapKey') or roll random. Daily seed mode rolls
  // deterministically from today's date.
  if (mission.useDailySeed || mission.dailySeed) {
    rngSeed(dailySeed());
    STATE.useDailySeed = true;
  } else {
    rngSeed((Math.random() * 0xffffffff) >>> 0);
    STATE.useDailySeed = false;
  }
  const mapKeys = Object.keys(MAP_ARCHETYPES);
  const mapKey = mission.mapKey || rngPick(mapKeys);
  STATE.map = MAP_ARCHETYPES[mapKey];
  // Roll weather (skip for first mission for fairness)
  const weatherKeys = mission.id === 1 ? ['clear'] : Object.keys(WEATHERS);
  STATE.weatherKey = mission.weatherKey || rngPick(weatherKeys);
  // Reset map-derived state
  applyWeatherToScene();   // updates fog density on existing scene
  // Need to rebuild ALL field elements since map changed.
  // Easiest: re-init the whole Three.js scene.
  reinitFieldForMap();
  clearBattle();
  rebuildTrees();
  createTowers();
  ...
  STATE.passives = {};
  normalizeDeck();
  // Pre-match toast: show map + weather rolled
  setTimeout(() => {
    showToast(STATE.map.name + ' • ' + getWeather().name + (STATE.useDailySeed ? ' • DAILY' : ''));
  }, 250);
  const fk = STATE.progress.playerFaction;
...
// Pass-C: Daily Battle — same map+weather all day, deterministic from date
function dailyBattle() {
  const eFK = ['empire','mercs','cult','rebels'].filter(f => f !== STATE.progress.playerFaction)[0];
  startMatch({ id:0, name:'Daily Battle', desc:'Same map for everyone today', enemy:'Daily Foe',
    enemyFaction:eFK, aiSpeed:0.85, aiSmart:0.65, enemyHpMul:1, mode:'1v1', useDailySeed:true });
}
